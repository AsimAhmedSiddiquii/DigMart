...
router.get("/(:sellerslugID)", useLocals, async (req, res, next) => {
    var id = req.params.sellerslugID;
    const seller = await Seller.findOne({ slugID: id }).select('slugID busLogo busName busMobile busAddress busZip plan').exec();
    ...
    if (req.session.userID) {
        ...
        const check = await Subscribe.find({
            userID: req.session.userID,
            sellerID: seller._id,
        });
    ...
    var proDocs = await Products.find({
        sellerID: seller._id,
        status: "Verified",
